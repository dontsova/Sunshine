...
public class WeatherProvider extends ContentProvider {
    ...
    private static final String sLocationSettingsWithStartDaySelection =
            WeatherContract.LocationEntry.TABLE_NAME+
                    "." + WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING + " = ? AND " +
                    WeatherContract.WeatherEntry.COLUMN_DATE + " >= ?";

    //location.location_setting = ? AND date = ?
    private static final String sLocationSettingsAndDaySelection =
            WeatherContract.LocationEntry.TABLE_NAME+
                    "." + WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING + " = ? AND " +
                    WeatherContract.WeatherEntry.COLUMN_DATE + " = ?";
    ...
    private Cursor getWeatherByLocationSettings(Uri uri, String[] projection, String sortOrder) {
        ...
        if (startDate == 0){
            ...
            selection = sLocationSettingsWithStartDaySelection;
        }

        return sWeatherByLocationSettingsQueryBuilder.query(mOpenHelper.getReadableDatabase(),
                projection,
                selection,
                selectionArgs,
                null,
    ...
    private Cursor getWeatherByLocationSettingAndDate(
        ...
        String locationSettings = WeatherContract.WeatherEntry.getLocationSettingFromUri(uri);
        long date = WeatherContract.WeatherEntry.getDateFromUri(uri);

        return sWeatherByLocationSettingsQueryBuilder.query(mOpenHelper.getReadableDatabase(),
                projection,
                sLocationSettingsAndDaySelection,
